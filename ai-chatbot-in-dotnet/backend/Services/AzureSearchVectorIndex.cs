...
public class AzureSearchVectorIndex(SearchClient searchClient, SearchIndexClient indexClient, string indexName) : IVectorIndex
{
    ...
    private async Task EnsureIndexAsync()
    {
        ...
        // FieldBuilder doesn't map ReadOnlyMemory<float> + [VectorSearchField] to
        // Collection(Edm.Single); it falls back to Edm.ComplexType, which Azure Search
        // rejects on upsert. Build that one field manually instead.
        var fields = new FieldBuilder().Build(typeof(ChunkSearchDocument)).ToList();
        var valuesIndex = fields.FindIndex(f => f.Name == nameof(ChunkSearchDocument.Values));
        fields[valuesIndex] = new SearchField(nameof(ChunkSearchDocument.Values), SearchFieldDataType.Collection(SearchFieldDataType.Single))
        {
            IsSearchable = true,
            IsHidden = false,
